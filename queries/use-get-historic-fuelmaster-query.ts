...
import { getHistoricalFuelMasterAlertsListApi } from "@/lib/api";
...
export const getHistoricFuelmasterListQueryKey = (
  ...rest: (string | number | Date)[]
): readonly [string, ...(string | number | Date)[]] => [
  "get-alarms-fuelmaster-historic",
  ...rest
];

export const useHistoricFuelmasterListQuery = ({ limit = 10, page = 1 }: { limit?: number; page?: number }) => {
  const { token } = useAuth();
  ...
  return useInfiniteQuery({
    queryKey: getHistoricFuelmasterListQueryKey(
      token ?? "",
      vessel?.id ?? "",
      from_date ?? "",
      to_date ?? "",
    ...
    queryFn: () =>
      fetchHistoricFuelmasterAlarmsList({ id: vessel?.id ?? null, from_date: from_date ?? null, to_date: to_date ?? null, sort, limit, page, search }, token),
...
const fetchHistoricFuelmasterAlarmsList = async (
  {
    id,
    from_date,
    to_date,
  ...
  const res = await getHistoricalFuelMasterAlertsListApi(
    {
      id,
