...
export const useHistoricAlarmsListQuery = ({
  ...
  return useQuery({
    queryKey: getHistoricAlarmsListQueryKey(
      String(vessel?.id) ?? "",
      String(sort),
      String(to_date),
      String(from_date),
...
const fetchHistoricAlarmsList = async (
  id: number | null,
  from_date: string,
  to_date: string,
  sort: "asc" | "desc",
  ...
  const res = await getHistoricAlarmsListApi(
    id,
    from_date,
    to_date,
    sort,
