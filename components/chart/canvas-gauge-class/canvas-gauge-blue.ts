...
const TICK_LABEL_DIGITT_HRESHOLD = 4;

export class CanvasGaugeBlueRenderer extends CanvasGaugeRenderer {
  private theme = {
    c1FillStyle: "#151A2F",
    c2StrokeStyle: "#00A9FF",
    c3StrokeStyle: "#0085C9",
    c4FillStyle: "#1A2755",
    c5Grad1: "#4A6EDF",
    c5Grad2: "#1A2755",
    cValueStrokeStyle: "#00A9FF",
    fontLabelFill: addOpacity("#FFFFFF", 70),
    fontTickLabelScaleFill: addOpacity("#FFFFFF", 65),
    fontLabelShadow: "#0085C9",
    fontUnitFill: addOpacity("#FFFFFF", 90),
    fontUnitShadow: "transparent",
    fontValueFill: "#FFFFF",
    fontValueShadow: "transparent",
    strokeStyle: "#00A9FF",
  ...
  public render() {
    ...
    this.drawUnitText(cX, cY);
    this.drawTickLabelScaleText(cX, cY + this.baseUnit * 33);
  ...
  private drawC5LayerWithLabels(
    ...
    const grad = this.createRadialGradient(cx, cy, 0, r, [
      { stop: 0, color: this.theme.c5Grad1 },
      { stop: 1, color: this.theme.c5Grad2 }
    ]);
    ...
    for (let i = 0; i < numLabels; i++) {
      const fontSize = this.baseUnit * 4; // Reduced from 4.7
      const valueLabel =
      ...
      const labelRadius = r2 + this.baseUnit * 4; // Reduced from 5 to bring numbers closer
  ...
  private drawUnitText(cx: number, cy: number) {
    ...
    this.ctx.fillText(this.unit, cx, cy + this.baseUnit * 11);
    this.ctx.shadowBlur = 0;
    this.ctx.shadowColor = this.theme.fontUnitShadow;
  }
